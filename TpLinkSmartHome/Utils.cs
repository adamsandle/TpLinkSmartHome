...
namespace TpLinkSmartHome
{
    ...
    {
        internal static T SendToSmartPlugOrSwitch<T>(string ip, string jsonPayload, int port = 9999)
        {
            return SendToSmartDevice<T>(ip, jsonPayload, SocketType.Stream, ProtocolType.Tcp, port);
        }

        private static T SendToSmartDevice<T>(string ip, string jsonPayload, SocketType socketType, ProtocolType protocolType, int port)
        {
            using (var sender = new Socket(AddressFamily.InterNetwork, socketType, protocolType))
            {
                ...
                if (bytesLen > 0)
                {
                    return JsonConvert.DeserializeObject<T>(Decrypt(buffer.Take(bytesLen).ToArray(), protocolType == ProtocolType.Tcp));
                }
                else
                {
